...
为了避免 `/chat` 等待所有分段语音生成完才返回，当前 TTS 策略是：

```text
Agent 生成 dialogue_segments
↓
后端只同步生成第 0 段 TTS
↓
/chat 立即返回第 0 段 inline 音频 + 后续段 audio_url
↓
后续段 TTS 在后台继续生成
↓
Godot 播到后续段时 GET /tts/audio/{cache_key}
↓
如果 WAV 还没落盘，后端短暂等待后台任务完成
```

这能把“首句开口”从“模型耗时 + 所有 TTS 段耗时”降低到“模型耗时 + 第一段 TTS 耗时”。后续段仍能保持一句字幕对应一句语音。

注意：VOICEVOX 的 `/audio_query` 只是生成语音参数，真正耗时通常在 `/synthesis`。GPU 版可以减少推理时间，但不代表任何句子都会毫秒级完成；首次加载、句子长度、speaker、CPU/GPU 后端、缓存命中都会影响耗时。缓存命中时后端通常能在几十毫秒内返回。
